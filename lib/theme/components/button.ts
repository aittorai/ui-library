...
const getColors = (
  ...
  const bgColor = c === 'appYellow' ? `${c}.500` : `${c}.400`;
  const bgBaseHover = 'base.300';
  const bgColorHover = c === 'appYellow' ? `${c}.300` : `${c}.300`;
  ...
  if (variant === 'outline') {
    return {
      ...notSolidFg,
      ...noBg,
      borderColor: c === 'appYellow' ? `${c}Alpha.500` : `${c}Alpha.400`,
      borderColorDisabled: 'base.600',
      borderColorHover: c === 'appYellow' ? `${c}Alpha.700` : `${c}Alpha.600`,
    };
  }
